...
public:
    ...
    enum stage {
        START, WFK, PRE_FIXATION, DRAW_FIXATION, FIXATION, CLEAR_FIXATION, BLANK, DRAW_STIM, STIM, FEEDBACK, FINAL_CLEAR
    };
        ...
        switch (state) {
            ...
            case START:
                ...
                delay = map<string, int>{
                    {"random_delay", random_int(DELAY_MIN, DELAY_MAX)},
                    {"fixed_delay", (DELAY_MIN + DELAY_MAX) / 2}}
                [session_cnd()];
            ...
            case DRAW_STIM:
                ...
                center(rect);
                rect.setFillColor(map<string, Color>{
                    {"fixed_delay", Color(intensity, 0, 0)},
                    {"random_delay", Color(0, intensity, 0)}}
                [session_cnd()]);
                clear(bg);
                draw(rect);
                display();
...
int main() {
    ...
    unique_ptr<SRT>(new SRT())->run("srt_test1",{
        {"stage",
            {"test"}}}, 3);
    unique_ptr<Instruction>(new Instruction("I jeszcze jeden etap właściwy..."));
    unique_ptr<SRT>(new SRT())->run("srt_test2",{
        {"stage",
            {"test"}}}, 3);
